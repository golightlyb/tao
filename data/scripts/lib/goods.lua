...
xGood["XCarbs"]             ={name="Carbohydrates",     description="Staple food, like potatoes or any xenograin, dried and boxed."}
...
xGood["XSystem"]            ={name="System",            description="Integrated technological components, like a shield generator."}
...
xGood["XWarhead"]           ={name="Warread",           description="A weapon of mass destruction."}
...
local tmpI = string.byte("I")
local tmpD = string.byte("D")
local tmpB = string.byte("B")
local tmpC = string.byte("C")
local tmpM = string.byte("M")
local tmpT = string.byte("T")

local function xUnpackString(xs)
    local isBasic      = false
    local isConsumer   = false
    local isIndustrial = false
    local isMilitary   = false
    local isTechnology = false
...
for key, val in pairs(xGood) do
    ...
    local size = 0.1 * s.size
    local price = size * s.price * math.pow(2, s.level)
    price = price * 2 -- rebalance ours vanilla avorion
    ...
    if isIllegal then price = price * 2.0 end
    price = math.floor(s.price + 0.5)
    
    goods[key] = {
        name            = g.name,
        plural          = g.name,
        descriptrion    = g.description,
        ...
        illegal         = illegal,
        dangerous       = dangerous,
        tags            = xUnpackString(s.tags),
        chains          = xUnpackString(s.chains),
    }
end
